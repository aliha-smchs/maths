...
def alldata():
    ...
    # Sidebar for customization
    subjects_all = "Social Studies"
    year_group = ['Year 3', 'Year 4', 'Year 5', 'Year 6', 'Year 7', 'Year 8',
                  'Year 9', 'Year 10']
    ...
    data_2023_filtered_all[subjects_all] = pd.to_numeric(
        data_2023_filtered_all[subjects_all].apply(lambda x: x.strip() if isinstance(x, str) else x),
        errors='coerce').fillna(
        0).astype(int)
    data_2024_filtered_all[subjects_all] = pd.to_numeric(
        data_2024_filtered_all[subjects_all].apply(lambda x: x.strip() if isinstance(x, str) else x),
        errors='coerce').fillna(
        0).astype(int)
